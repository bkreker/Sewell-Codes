...
namespace QueryMining
{
    public partial class AnalyzeForm : Form
    {
        ...
        private DataGridViewColumn QueryColumn { get { return dgvInFile.Columns[_queryColIndex]; } }
        private DataGridViewColumn QueryCountColumn
        {
            get { return dgvInFile.Columns[_queryCountColIndex]; }
        }
            ...
            try
            {
                if (dgvOutFile.InvokeRequired)
                {
                    dgvOutFile.Invoke(new MethodInvoker(delegate
                    {
                        ...
                        dgvOutFile.Sort(dgvOutFile.Columns[StatDataTable.QueryCountCol], ListSortDirection.Descending);
                        dgvOutFile.Refresh();
        ...
        private void SaveData()
        {
            try
            {
                ...
                outFile.WriteLine(string.Join(",", StatDataTable.Headers));
        ...
        private void Analyze()
        {
            ...
            try
            {
                var preExistingKeys = (from DataRow r in _inFileTable.Rows
                                       select r.ItemArray).ToList();

                preExistingKeys.ForEach(row => _checkedKeys.Add(row[_queryColIndex].ToString(), true));
            ...
            if (wordString != "")
            {
                try
                {
                    ...
                    if (existingKeys.Count == 0)
                    {
                        ...
                        {
                            newRow = StatDataTable.AggregateRows(query, wordString, existingRows, ref _operationCancelled);
                        }
                        else
                        {
        ...
        private void btnImport_Click(object sender, EventArgs e)
        {
            ...
            if (form.DialogResult == DialogResult.OK)
            {
                ...
                this._queryColIndex = StatDataTable.QueryCol;
                this._queryCountColIndex = StatDataTable.QueryCountCol;
                try
                {
