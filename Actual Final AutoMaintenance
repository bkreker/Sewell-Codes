...
var reducedBids = [['Reduced Bids:'],TITLES];
var reducedNum = 0;
var increasedBids = [['Increased Bids:'],TITLES];
var increasedNum = 0;
var errorNum = 0;
var ERROR_LOG = [['Errors:'],['\nCampaign','AdGroup','Keyword','MatchType']];
var pausedKWs = [['Paused Keywords:'],TITLES];
var pausedNum = 0;
...
function GetStat(campaign, adGroup, kwId, _stat){
  ...
  .get();
  var Stat = 0;
  if (_stat === 'Cost'){Stat = report.next().getCost();}
  if(_stat === 'AvgCPC'){Stat = report.next().getAvgCPC();}
  if(_stat === 'Conversions'){Stat = report.next().getConversions();}
  return Stat;
}
