...
function main() {
  ...
  for(var level in levels_to_tag) {
    var report = getContentRows(levels_to_tag[level]);
    var entity_map = buildEntityMap(levels_to_tag[level]);
    for(var parent_id in entity_map) {
      ...
      for(var i in stats_list) {
		  var entity = entity_map[parent_id]['entity'];
		  var stat_key = stats_list[i];
        var mean = getMean(child_list,stat_key);
        var stand_dev = getStandardDev(child_list,mean,stat_key);
        var label_name = stats_list[i]+"_anomaly";
		addLabelToAnomalies(entity,mean,stand_dev,stat_key,label_name,level);
      }
    }
    sendResultsViaEmail(report,levels_to_tag[level]);
  }
}
...
function sendResultsViaEmail(report,level) {
  ...
  if(rows == 0) { return; }
  var options = { 
	attachments: [Utilities.newBlob(report, 'text/csv', level+"_anomalies_"+_getDateString()+'.csv')] 
  ...
  var subject = 'AdWords Alert: Abnormal ' + _initCap(level) + ' Entities Report - ' + _getDateString();
  for(var i in TO) {
...
function addLabelToAnomalies(entites,mean,sd,stat_key,label_name,level) {
  ...
  for(var i in entites) {
    ...
    if(sd != 0 && deviation/sd >= STANDARD_DEVIATIONS && !hasLabelAlready(entity, label_name)) {
...
function getStatsMap(stats) {
  return { // You can comment these out as needed.
          ...
          conv_rate : stats.getConversionRate(),
...
// Helper function to check if the entity already has the label
 function hasLabelAlready(entity, label_name){  
	try{
		var level = entity.getEntityType();
		if (level === 'Ad'){
			var labels = AdWordsApp.ads()
			.withCondition("Headline CONTAINS_IGNORE_CASE '"+ entity.getHeadline() + "'")
			...
			var labels = AdWordsApp.adGroups()
			.withCondition("Name CONTAINS_IGNORE_CASE '"+ entity.getAdgroup().getName + "'")
			.withCondition("LabelNames CONTAINS_ANY ["+ label_name +"]")
			.get();
			...
			var labels = AdWordsApp.campaigns()
			.withCondition("Name CONTAINS_IGNORE_CASE '"+ entity.getCampaign().getName() + "'")
			.withCondition("LabelNames CONTAINS_ANY ["+ label_name +"]")
			.get();
