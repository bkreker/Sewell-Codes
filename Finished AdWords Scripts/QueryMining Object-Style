...
function main() {
    ...
    var queryReport = AdWordsApp.report(
        ...
        "AND CampaignName CONTAINS_IGNORE_CASE '" + campaignNameContains + "' " +
        "DURING " + dateRange);
    //campaignSearchWords["CampaignName"][currentWord[w]][statColumns[i]];
    ...
    while (queryRows.hasNext()) {
        ...
        var doneWords = [];
        Logger.log("Writing query to object on line 232.");
        if (campaignSearchWords.campName[] == undefined) {
            campaignSearchWords[queryRow["CampaignName"]].currentWords.query = [searchQuery];
        }
    ...
    for (var campaign in campaignSearchWords) {
        for (var word in campaignSearchWords[campaign]) {
            ...
            if (campaignSearchWords[campaign][word]["Clicks"] < clickThreshold) {
                continue;
            }
            if (campaignSearchWords[campaign][word]["Cost"] < costThreshold) {
                continue;
            }
            if (campaignSearchWords[campaign][word]["ConvertedClicks"] < conversionThreshold) {
                continue;
            }
    ...
    for (var i = 0; i < totalSearchWordsKeys.length; i++) {
        ...
        if (totalSearchWords[word]["Impressions"] < impressionThreshold) {
            continue;
        }
        if (totalSearchWords[word]["Clicks"] < clickThreshold) {
            continue;
        }
        if (totalSearchWords[word]["Cost"] < costThreshold) {
            continue;
        }
        if (totalSearchWords[word]["ConvertedClicks"] < conversionThreshold) {
            continue;
        }
