...
var TITLES = ['\SKU', 'Campaign', 'AdGroup', 'Keyword', 'MatchType', 'QS', 'OldBid', 'NewBid', 'BidChange', 'KwBidMax', 'AvgCPC', 'Cost', 'ConversionValue', 'NetProfit', 'Conversions', 'NP_PerConv', 'CPA', 'MaxCPA', 'kwNum', 'ROI', 'Criteria', 'LifeTimeProfit', 'Action'];
...
function buildSelector(dateRange, isString) {
	...
	if (isString) {
		...
		// print(JSON.stringify(dateRange, null,'\t'));
		var to = dateRange.toObj;
		var from = dateRange.fromObj;
		kw_iter = AdWordsApp.keywords()
			.forDateRange(dateRange.string)
			.withCondition("Status = ENABLED")
			.withCondition("AdGroupStatus = ENABLED")
			.withCondition("CampaignStatus = ENABLED")
			.withCondition("Cost > 0")
...
function infoReduced(item) {
	...
	var action = 'Reduced,';
	info(action + toLog);
	reducedBids = reducedBids.concat([item, action]);
}
...
function infoIncreased(item) {
	...
	var action = 'Increased,';
	info(action + toLog);
	increasedBids = increasedBids.concat([item, action]);
}
...
function infoPaused(item) {
	...
	var action = 'Paused,';
	info(action + toLog);
	pausedKWs = pausedKWs.concat([item, action]);
}
