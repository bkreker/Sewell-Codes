...
var TITLES = ['\nAction', 'SKU', 'Campaign', 'AdGroup', 'Keyword', 'MatchType', 'QS', 'OldBid', 'NewBid', 'BidChange', 'KwBidMax', 'AvgCPC', 'Cost', 'ConversionValue', 'NetProfit', 'Conversions', 'NP_PerConv', 'CPA', 'MaxCPA', 'kwNum', 'ROI', 'Criteria', 'LifeTimeProfit'];
...
function infoReduced(item) {
	...
	var action = 'Reduced,';
    info(action + toLog);
    reducedBids = reducedBids.concat([action, item]);
}
...
function infoIncreased(item) {
	...
	var action = 'Increased,';
    info(action + toLog);
    increasedBids = increasedBids.concat([action, item]);
}
...
function infoPaused(item) {
	...
	var action = 'Paused,';
    info(action + toLog);
    pausedKWs = pausedKWs.concat([action, item]);
}
