...
export function _getDateString() {
    try {
        var today = new Date();
        var timeZone = AdWordsApp.currentAccount().getTimeZone();
        var dayFormat = "MM-dd-yyyy";
...
export function _todayIsMonday() {
    try {
...
export function sendResultsViaEmail(report, level) {
    try {
...
export function EmailResults(ReportName) {
    try {
        var _emails = EMAILS;
