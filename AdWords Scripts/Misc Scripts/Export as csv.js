...
function main(){
  var level_to_tag = 'keyword'; // 'adgroup','ad'  
  var report = getContentRows(level_to_tag);
...
function toReportRow(entity,level,label_name) {
  var ret_val = [entity.getCampaign().getName()];
...
function increaseCheapConversions() {
  ...
  //For keywords with less than $5 CPA, let's pump those bids up by 35%
  var AMAZING_COST_PER_CONV = 5;
  var AMAZING_BID_INCREASE_AMOUNT = .35;
  
  //For keywords with between $5 and $8 CPAs, we will only increase the bids by 20%
    ...
    if(bidStrategy(bidType)){
      ...
      if(conversions > 0) {
        ...
        if(cost_per_conversion <= AMAZING_COST_PER_CONV) {
			...
			kw.setMaxCpc(newBid); 
			var message =  kw + ";" +adGroup + ";"+oldBid + ';' + newBid;		  
